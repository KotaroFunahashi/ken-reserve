# 要件定義書：ken-reserve (けんぼう家庭教師予約システム)

## 1. プロジェクト概要

* **システム名:** ken-reserve
* **目的:**
  * LINEベースの予約調整を自動化し、講師の空き時間をWeb公開・予約可能にする。
  * Java 21 / Spring Boot 4.x 系を用いたプログラミング学習と実用化。
* **コンセプト:** 最小限の画面数で、最速で立ち上げる（MVP開発）。
* **ターゲット:** 家庭教師（講師）、生徒、保護者。

## 2. ユーザーロール

| ロール | 説明 |
| --- | --- |
| **管理者** | ユーザーの作成・管理、システム全体の監視。 |
| **講師** | スケジュールの登録（休暇設定）、予約の承認・キャンセル操作。 |
| **生徒/保護者** | 空き枠の確認、予約申請、自身の予約キャンセル。 |

## 3. 業務ロジック・制約ルール

AIが実装を誤らないよう、カレンダーのロジックを厳密に定義します。

* **時間枠 (Slot):** 09:00 〜 21:00 の間、30分刻みで表示。
* **授業定義:** 1授業 **60分固定**。
* **バッファルール:**
  * 予約が入った場合、その**前後30分**も「✕（予約不可）」として扱う。
  * 例：10:00枠に予約が入った場合、09:30枠、10:00枠、10:30枠、11:00枠が埋まる。
  * **バッファの衝突判定:**
    * **APPROVED予約との衝突:** 新規予約のバッファ圏 [start-30分, end+30分) が、承認済み予約のバッファ圏と半開区間で重複する場合は予約不可。
    * **PENDING予約との衝突:** 時間帯が直接重複する場合のみ予約不可。バッファのみの重複は許可する。
    * 隣接（境界が同一）は常に許可。
  * **休暇にはバッファは適用されない。** ただし、予約時間帯が休暇と直接重複する場合は予約不可。
* **スロットブロック対象ステータス:** `PENDING` および `APPROVED` の予約がカレンダー上でスロットをブロック（✕表示）する。`REJECTED` はブロックしない。
* **予約フロー:**
  1. 生徒が「○」をクリックして申請（ステータス：`PENDING`）。
  2. 講師が一覧から「承認」または「キャンセル」を選択。
  3. 生徒がホーム画面で結果を確認し、「確認済み」ボタンを押下してフロー終了。

## 4. 画面設計 (SSR / UI Flow)

### 4.1 共通ヘッダー

* システム名（けんぼう家庭教師予約システム）
* メニューリンク（ホーム、生徒管理、予約管理、ログアウト）

### 4.2 ホーム画面 (`/`)

* **カレンダー表示:** 1週間分。30分単位のグリッド。
  * `○`: 空き。生徒がクリックで予約申請。
  * `-`: 講師の休暇。
  * `✕`: 予約済み（講師には予約者名を表示）。
* **講師用セクション (下部):**
  * 未回答予約リスト（承認・キャンセルボタン付き）。
* **生徒用セクション (上部):**
  * 予約結果の通知（承認された/キャンセルされた通知、確認ボタン）。

### 4.3 ユーザー管理画面 (`/users`)

* **ロール別権限:**

| 操作 | ADMIN | TUTOR | STUDENT |
| --- | --- | --- | --- |
| 一覧表示 | 全ユーザー | 自分 + 担当生徒のみ | 自分 + 担当講師のみ |
| 作成 | 全ロール | 担当生徒のみ | 不可 |
| 編集 | 全ユーザー (ロール変更可) | 自分のみ | 自分のみ |
| 削除 | 全ユーザー (自分以外) | 担当生徒のみ  | 不可 |
| 担当講師設定 | 全TUTORから選択 (作成・編集とも可) | 自分のみ (作成時のみ) | 不可 |

### 4.4 予約管理画面 (`/reservations`)

* 講師: 全予約のCRUD。
* 生徒: 自身の予約一覧、前日までキャンセル（削除）可能。

## 5. データモデル (ER図)

`Reservations` テーブルで「予約」と「休暇」を共通管理します。

```mermaid
erDiagram
    USERS ||--o{ RESERVATIONS : "makes"
    USERS {
        Long id PK
        String loginId "ログインID (必須)"
        String password "ハッシュ化パスワード (必須)"
        String display_name "表示名 (必須)"
        String role "ADMIN, TUTOR, STUDENT"
        Datetime created_at "作成日時"
        Datetime updated_at "更新日時"
    }
    RESERVATIONS {
        Long id PK
        Long tutor_id FK "教師ID (必須)"
        Long user_id FK "生徒ID (休暇の場合はnull)"
        String type "RESERVATION / VACATION"
        String status "PENDING / APPROVED / REJECTED"
        DateTime start_at "開始日時"
        DateTime end_at "終了日時"
        Datetime created_at "作成日時"
        Datetime updated_at "更新日時"
    }

```
